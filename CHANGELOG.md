...
- **Completed and deployed the Homepage dashboard** (VMID 121, `192.168.1.121`, `automation-utilities` band) at `dashboard.lab.jaysynclab.com`, closing [JaySync-Lab#11](https://github.com/JaySync-Lab/JaySync-Lab/issues/11). Full writeup on [`docs/services/homepage-dashboard.mdx`](docs/services/homepage-dashboard.mdx), now flipped from `draft` to `published`
  ...
  - Gotcha: the skeleton config's Proxmox widget fields (`password`/`apiToken` split) didn't match Homepage's actual schema — correct form is `username: user@realm!tokenname` plus a single `password` holding the token secret
  - Gotcha: newer Homepage builds require `HOMEPAGE_ALLOWED_HOSTS` set explicitly or every request 500s with "Host validation failed" — and the container needs a full recreate, not just a restart, for it to take effect
  - Verified end-to-end, not just "configured without erroring": Pi-hole app password and the HA token each confirmed directly against their own APIs, then the full DNS → reverse-proxy (wildcard TLS) → container path tested at `200`
- **Fixed a real, pre-existing gap**: the router's DHCP was never configured to hand out Pi-hole as the LAN's DNS server — every device on the home VLAN was using the router itself for DNS, meaning Pi-hole's ad-blocking/local-DNS function was inactive network-wide, not just for `*.lab.jaysynclab.com`. Found via `nslookup` returning `NXDOMAIN` against the router but resolving correctly against Pi-hole directly. Fixed in two parts: (1) router's DHCP DNS field pointed at `192.168.1.101`, (2) the router was also advertising itself as an IPv6 DNS server via Router Advertisements, which Windows prefers over IPv4 — had to disable IPv6 on the router's LAN side (requires a reboot to fully apply) before DNS resolution was consistent
- Host stability: the Proxmox host went intermittently offline for over an hour during this session (Tailscale reporting it flapping), same unexplained pattern as an earlier multi-day outage. Confirmed back online and stable before resuming work; root cause remains undiagnosed and is a physical/hardware concern, not something fixable remotely
- Filed [jaysync-lab-playground#39](https://github.com/JaySync-Lab/jaysync-lab-playground/issues/39): the "Notify me" outage-subscribe form silently fails — root-caused to `Redis.fromEnv()` expecting `UPSTASH_REDIS_REST_URL`/`UPSTASH_REDIS_REST_TOKEN`, but only the older `KV_REST_API_URL`/`KV_REST_API_TOKEN` naming is configured on Vercel. Not yet fixed
- Redesigned the public site's network topology diagram (`jaysync-lab-site`) to group nodes by VMID band instead of a hardcoded per-service position/color map — the old approach silently stacked any node it didn't recognize at the same coordinates with the same grey color. New version derives layout from each node's `band` field and gives each band a distinct branch/color, with per-node color variance layered on top; also given significantly more page real estate on both the landing page and `/architecture` per direct feedback that it's a centerpiece, not a small inset
